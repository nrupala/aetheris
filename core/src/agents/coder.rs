...
impl CoderAgent {
    pub fn new(model: String, system_prompt: String,
        ...
        Self {
            base: BaseAgent::new(
                AgentRole::Coder, model,
                if system_prompt.is_empty() { "You are a code generation agent. Write clean, well-documented, production-ready code.".to_string() } else { system_prompt },
...
impl Agent for CoderAgent {
    ...
    fn role(&self) -> AgentRole { AgentRole::Coder }
    ...
    async fn execute(&mut self, task: &str, context: &serde_json::Value) -> AgentResult {
        ...
        if !self.base.check_policy("read", task).await {
            ...
            return AgentResult {
                agent_id: self.base.id.clone(), role: "coder".to_string(),
        ...
        let code_context = context.get("rag_answer").and_then(|v| v.as_str()).unwrap_or("");
        let kg_context = context.get("kg_context").and_then(|v| v.as_str()).unwrap_or("");
        let workspace = context.get("workspace").and_then(|v| v.as_str()).unwrap_or("/workspace");
        ...
        let code_output = self.base.call_llm(vec![
            serde_json::json!({"role": "system", "content": &self.base.system_prompt}),
            serde_json::json!({"role": "user", "content": &prompt}),
        ], 0.1, 4096).await.unwrap_or_else(|e| format!("// Code generation failed: {}", e));
