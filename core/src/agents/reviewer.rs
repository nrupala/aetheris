...
impl ReviewerAgent {
    pub fn new(model: String, system_prompt: String,
        ...
        Self {
            base: BaseAgent::new(
                AgentRole::Reviewer, model,
                if system_prompt.is_empty() { "You are a code review agent. Evaluate content for correctness, quality, and security.".to_string() } else { system_prompt },
...
impl Agent for ReviewerAgent {
    ...
    fn role(&self) -> AgentRole { AgentRole::Reviewer }
    ...
    async fn execute(&mut self, task: &str, context: &serde_json::Value) -> AgentResult {
        ...
        if !self.base.check_policy("evaluate", task).await {
            ...
            return AgentResult {
                agent_id: self.base.id.clone(), role: "reviewer".to_string(),
        ...
        let content = context.get("content").and_then(|v| v.as_str()).unwrap_or("");
        let standards_context = context.get("rag_answer").and_then(|v| v.as_str()).unwrap_or("");
        let kg_history = context.get("kg_context").and_then(|v| v.as_str()).unwrap_or("");
        let criteria = context.get("criteria")
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter().filter_map(|c| c.as_str().map(String::from)).collect::<Vec<_>>())
            .unwrap_or_else(|| vec!["correctness".to_string(), "quality".to_string(), "security".to_string()]);
        ...
        let review: serde_json::Value = serde_json::from_str(&review_result)
