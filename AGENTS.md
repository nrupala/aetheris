...
### OPA Phase 3 seam (NOT built — build after P2)
...
```
Prereqs already in place: `AppState.opa_enforce` (P2), `identity_to_role`/`AuthzInput` (P1). Remaining: `is_sensitive(method, path)` restricting to POST/PUT/DELETE + `/upload,/ingest,/keys,/bridge,/task,/workflow,/sync/upload` (GET/static/panels stay under CF Access).
