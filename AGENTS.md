...
- **RAG config correction (2026-08-07)** ✅ — on-box `/data/vault/rag_config.json` had been persisted with `qwen3:8b` as `embed_models`/`query_model`/`reranker_model` + `reranker_enabled=true`. **qwen3:8b is a generation model, not an embedding model** → "No embedding in response from qwen3:8b" on ingest. Corrected on box to `embed_models=["nomic-embed-text"]`, `query_model="phi4-mini"`, `reranker_enabled=false` (Ollama 0.24 has no `/api/rerank`) + restart; verified 768-dim embed + ingest now indexes. **File-type note:** the supported ingest extensions (PDF→text-extract, TXT/MD/HTML/JSON/CSV/XML/YAML/TOML + code PY/RS/JS/TS/C/CPP/…/ADA) are all **text/code formats** — chunker + `nomic-embed-text` are format-agnostic, so one small embed model covers every listed type (no per-format model needed). PDFs require extractable text; scan/image-only PDFs yield 0 chunks.
- **P5 JWT enforcement proven end-to-end (2026-08-07)** — a real browser panel write (`POST /ingest/file` as `nrupalakolkar@gmail.com`) produced an OPA `result:true`/admin decision with **zero `CFJWT deny` lines**, confirming the signed CF Access JWT → admin → allow path under `CF_JWT_VERIFY=1`.
