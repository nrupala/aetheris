...
- **OPA Phase 3 — shadow middleware (blocks nothing)** ✅ — `opa_gate` (`axum::middleware::from_fn_with_state`) applied to the whole router right before `.with_state(state)`; evaluates every request, logs + bumps `aetheris_security_violations_total` on would-deny, and only 403s when `opa_enforce && is_sensitive(...)` (off this phase → shadow only). `access_role(email, Cf-Access-Client-Id)` → `identity_to_role` (admin / analyst / unknown); `is_sensitive` (POST/PUT/DELETE or `/upload,/ingest,/keys,/bridge,/task,/workflow,/sync/upload`). 6 new tests (shadow pass-through, would-deny logs+bumps+passes, allow pass, enforce-blocks-sensitive, is_sensitive, identity map).
