...
RESP=$(curl -s http://localhost:$CORE_PORT/status 2>/dev/null)
...
if curl -s http://localhost:$VICTORIA_PORT/health >/dev/null 2>&1; then
...
METRICS=$(curl -s http://localhost:$CORE_PORT/metrics 2>/dev/null)
...
HTML=$(curl -s http://localhost:$CORE_PORT/ 2>/dev/null)
...
echo "  To test: docker restart aetheris_core && sleep 10 && curl localhost:\$CORE_PORT/status"
